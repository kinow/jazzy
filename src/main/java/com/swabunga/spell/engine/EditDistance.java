 ...
 * word into the other, the lowest cost distance is returned. <br/>
...
public class EditDistance {
    ...
    public static final int getDistance(String word, String similar,
            int[][] matrix) {
        /*
         * JMH Again, there is no need to have a global class matrix variable in
         * this class. I have removed it and made the getDistance static final
         * DMV: I refactored this method to make it more efficient, more
         * readable, and simpler. I also fixed a bug with how the distance was
         * being calculated. You could get wrong distances if you compared
         * ("abc" to "ab") depending on what you had setup your COST_REMOVE_CHAR
         * and EDIT_INSERTION_COST values to - that is now fixed. WRS: I added a
         * distance for case comparison, so a misspelling of "i" would be closer
         * to "I" than to "a".
         */
        ...
        for (i = 1; i != a_size; ++i)
            matrix[i][0] = matrix[i - 1][0] + costOfInsertingSourceCharacter; // initialize
                                                                              // the
                                                                              // first
                                                                              // column

        for (j = 1; j != b_size; ++j)
            matrix[0][j] = matrix[0][j - 1] + costOfDeletingSourceCharacter; // initalize
                                                                             // the
                                                                             // first
                                                                             // row
        ...
        for (i = 1; i != a_size; ++i) {
            ...
            for (j = 1; j != b_size; ++j) {
                ...
                if (sourceChar == otherChar) {
                    matrix[i][j] = matrix[i - 1][j - 1]; // no change required,
                                                         // so just carry the
                                                         // current cost up
                    continue;
                }
