 ...
 * href="http://aspell.sourceforge.net/metaphone/"
 * />http://aspell.sourceforge.net/metaphone/</a> DoubleMetaphone does some
 * processing, such as uppercasing, on the input string first to normalize it.
 * Then, to create the key, the function traverses the input string in a while
 * loop, sending successive characters into a giant switch statement. Before
 * determining the appropriate pronunciation, the algorithm considers the
 * context surrounding each character within the input string.
 * <p>
...
public class DoubleMeta implements Transformator {
     ...
     * word.
     */
    private static char[] replaceList = { 'A', 'B', 'X', 'S', 'K', 'J', 'T',
            'F', 'H', 'L', 'M', 'N', 'P', 'R', '0' };
    ...
    private static final String[] list15 = { "L", "R", "N", "M", "B", "H", "F",
            "V", "W", " ", "" };
    ...
    private static final String[] list39 = { "ES", "EP", "EB", "EL", "EY",
            "IB", "IL", "IN", "IE", "EI", "ER", "" };
    ...
    private static final String[] list54 = { "L", "T", "K", "S", "N", "M", "B",
            "Z", "" };
    ...
    private static final String[] list75 = { "OO", "ER", "EN", "UY", "ED",
            "EM", "" };
    ...
    private static final String[] list90 = { "EWSKI", "EWSKY", "OWSKI",
            "OWSKY", "" };
    ...
    private final static boolean SlavoGermanic(String in) {
        if ((in.indexOf("W") > -1) || (in.indexOf("K") > -1)
                || (in.indexOf("CZ") > -1) || (in.indexOf("WITZ") > -1))
            return true;
        return false;
    }
     ...
     * @return
     */
    private final static boolean stringAt(String string, int start, int length,
            String[] list) {
     ...
     * minimized as much as possible, and consenants that have similiar sounds
     * are converted to the same consenant for example, 'v' and 'f' are both
     * converted to 'f'
     * 
    ...
    public final String transform(String word) {
        ...
        while (current < length) {
            ...
            case '\u00C7':
                ...
                break;
            case 'C':
                if ((current > 1) && !isVowel(in, current - 2, length)
                ...
                if (stringAt(in, current + 1, 2, list27))
                    current += 3;
                else if (stringAt(in, current + 1, 1, list28)
            ...
            case 'G':
                if (in.charAt(current + 1) == 'H') {
                    ...
                    if ((current > 1)
                            ...
                            || ((current > 2) && stringAt(in, current - 3, 1,
                                    list34))
                            || ((current > 3) && stringAt(in, current - 4, 1,
                                    list35))) {
                        current += 2;
                        break;
                    } else {
                        if ((current > 2) && (in.charAt(current - 1) == 'U')
                ...
                if (in.charAt(current + 1) == 'N') {
                    if ((current == 1) && isVowel(in, 0, length)
                            && !isSlavoGermaic) {
                ...
                if ((current == 0)
                        && ((in.charAt(current + 1) == 'Y') || stringAt(in,
                                current + 1, 2, list39))) {
                    MetaphAdd(primary, 'K');
                    current += 2;
                    break;
                }
                if ((stringAt(in, current + 1, 2, list40) || (in
                        .charAt(current + 1) == 'Y'))
            ...
            case 'J':
                if (stringAt(in, current, 4, list50)
                    ...
                    if ((current == 0) && (in.charAt(current + 4) == ' ')
            ...
            case 'L':
                if (in.charAt(current + 1) == 'L') {
                    if (((current == (length - 3)) && stringAt(in, current - 1,
                            4, list56))
                            || ((stringAt(in, last - 1, 2, list57) || stringAt(
                                    in, last, 1, list58)) && stringAt(in,
                                    current - 1, 4, list59))) {
                        MetaphAdd(primary, 'L');
                        current += 2;
                        break;
                ...
                break;
            case 'M':
                if ((stringAt(in, current - 1, 3, list60) && (((current + 1) == last) || stringAt(
                        in, current + 2, 2, list61)))
            ...
            case 'Q':
                ...
                break;
            case 'R':
                if ((current == last) && !isSlavoGermaic
            ...
            case 'S':
                ...
                if (((current == 0) && stringAt(in, current + 1, 1, list71))
            ...
            case 'W':
                ...
                if ((current == 0)
                        && (isVowel(in, current + 1, length) || stringAt(in,
                                current, 2, list89))) {
                    MetaphAdd(primary, 'A');
                }
                if (((current == last) && isVowel(in, current - 1, length))
                ...
                break;
            case 'X':
                if (!((current == last) && (stringAt(in, current - 3, 3, list93) || stringAt(
                        in, current - 2, 2, list94))))
                    MetaphAdd(primary, "KS");
